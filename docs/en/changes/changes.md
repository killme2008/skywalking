...
* Add GreptimeDB as a new storage option. Uses gRPC for high-throughput writes and MySQL protocol (JDBC) for queries. Supports all required DAO interfaces with native TTL, per-key indexed searchable-tag columns, and FULLTEXT log search.
