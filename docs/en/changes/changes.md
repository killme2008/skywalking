...
* Add GreptimeDB as a new storage option. Uses gRPC for high-throughput writes and MySQL protocol (JDBC) for queries. Supports all required DAO interfaces with native TTL, normalized append-only tables for searchable tags, query-driven secondary indexes, multi-frontend JDBC load balancing and failover, and FULLTEXT log search.
